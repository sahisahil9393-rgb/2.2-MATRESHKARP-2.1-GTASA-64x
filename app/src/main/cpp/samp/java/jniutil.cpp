...
void CJavaWrapper::ShowSpawn()
{
    ...
    env->CallVoidMethod(activity, s_showspawn);
...
void CJavaWrapper::HideSpawn()
{
    ...
    env->CallVoidMethod(activity, s_hidespawn);
        ...
        case 1:
            switch(action) {
               /* case 398:
                    ...
                    break;
                case 1:
                    pNetGame->SendChatCommand("/edgar");
                    break;
                case 2:
...
JNIEXPORT void JNICALL Java_ru_edgar_space_core_ui_usermenu_UserMenuManager_SendRpcMenu(JNIEnv* pEnv, jobject thiz, jint type)
{
    switch(type) {
        /*case 1:
            pNetGame->SendChatCommand("/gps");
...
JNIEXPORT void JNICALL Java_ru_edgar_space_SAMP_sendDialogResponse(JNIEnv* pEnv, jobject thiz, jint i3, jint i, jint i2, jbyteArray str)
{
    jboolean isCopy = true;
...
JNIEXPORT void JNICALL Java_com_nvidia_devtech_NvEventQueueActivity_setNativeDiaFLog(JNIEnv* pEnv, jobject thiz, jboolean b)
{
    /*if (pSettings)
    {
        pSettings->GetWrite().iDiaFLog = b;
    }*/
}
...
JNIEXPORT void JNICALL Java_ru_edgar_space_core_ui_spawnmenu_SpawnMenu_spawm(JNIEnv* pEnv, jobject thiz, jint idspawn)
{
    Chat::keyboardEvent("/spawn32");
...
JNIEXPORT jboolean JNICALL Java_com_nvidia_devtech_NvEventQueueActivity_getNativeDiaFLog(JNIEnv* pEnv, jobject thiz)
{
    if (pSettings)
    {
        //return pSettings->GetReadOnly().iDiaFLog;
    }
    return 0;
}
...
void CJavaWrapper::AddChatMessage(const char msg[], int color)
{
    //if(!thiz)return;
    char msg_utf[1024];
    //cp1251_to_utf8(msg_utf, msg, sizeof(msg_utf) - 1);

    jclass strClass = g_pJavaWrapper->GetEnv()->FindClass("java/lang/String");
    jmethodID ctorID = g_pJavaWrapper->GetEnv()->GetMethodID(strClass, "<init>", "([BLjava/lang/String;)V");
    jstring encoding = g_pJavaWrapper->GetEnv()->NewStringUTF("UTF-8");

    jbyteArray bytes = g_pJavaWrapper->GetEnv()->NewByteArray(strlen(msg));
    g_pJavaWrapper->GetEnv()->SetByteArrayRegion(bytes, 0, strlen(msg), (jbyte*)msg);
    jstring jtext = (jstring) g_pJavaWrapper->GetEnv()->NewObject(strClass, ctorID, bytes, encoding);
    ...
    env->CallVoidMethod(this->activity, this->s_AddChatMessage, jtext, color);
...
CJavaWrapper::CJavaWrapper(JNIEnv* env, jobject activity)
{
    ...
    s_showspawn = env->GetMethodID(nvEventClass, "showSpawnMenu", "()V");
    s_hidespawn = env->GetMethodID(nvEventClass, "hideSpawnMenu", "()V");

    s_connectedgar = env->GetMethodID(nvEventClass, "connectEdgar", "()V");
