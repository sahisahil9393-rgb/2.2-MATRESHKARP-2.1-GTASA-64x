...
void DoInitStuff()
{
	...
	if (!bNetworkInited && !bDebug)
	{
        ...
        env->CallVoidMethod(pJavaWrapper->activity, pJavaWrapper->s_connectedgar);
...
// Функция для преобразования jstring в std::string
...
extern "C" {
	...
	/*JNIEXPORT void JNICALL Java_ru_edgar_space_SAMP_onEventBackPressed(JNIEnv *pEnv, jobject thiz)
	{
		///if(pSettings)
		//{
	...
	JNIEXPORT void JNICALL Java_ru_edgar_space_ui_dialog_DialogManager_sendDialogResponse(JNIEnv* pEnv, jobject thiz, jint i3, jint i, jint i2, jbyteArray str)
	{
...
void InitGui()
{
	...
	std::string font_path = string_format("%sSPACE/fonts/%s", g_pszStorage, FONT_NAME);
...
void LogVoice(const char* fmt, ...)
{
	...
	if (flLog == nullptr && pszStorage != nullptr)
	{
		sprintf(buffer, "%sSPACE/%s", pszStorage, SV::kLogFileName);
