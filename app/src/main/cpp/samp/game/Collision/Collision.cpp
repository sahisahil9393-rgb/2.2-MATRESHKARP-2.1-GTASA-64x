...
bool CCollision::ProcessSphereBox(CColSphere const& sph, CColBox const& box, CColPoint & colp, float& minDistSq) {
    ...
    */
    ...
    for (auto i = 0; i < 3; i++) {
        axies[i] = sph.m_vecCenter[i] < box.m_vecMin[i] ? ClosestCorner::MIN :
                   sph.m_vecCenter[i] > box.m_vecMax[i] ? ClosestCorner::MAX :
                   ClosestCorner::INSIDE;
    }


    if(axies[0] == ClosestCorner::INSIDE && axies[1] == ClosestCorner::INSIDE && axies[2] == ClosestCorner::INSIDE) { // Sphere center is inside the bb
        ...
        const CVector p{
                axies[0] == ClosestCorner::MIN ? box.m_vecMin.x :
                axies[0] == ClosestCorner::MAX ? box.m_vecMax.x :
                sph.m_vecCenter.x,

                axies[1] == ClosestCorner::MIN ? box.m_vecMin.y :
                axies[1] == ClosestCorner::MAX ? box.m_vecMax.y :
                sph.m_vecCenter.y,

                axies[2] == ClosestCorner::MIN ? box.m_vecMin.z :
                axies[2] == ClosestCorner::MAX ? box.m_vecMax.z :
                sph.m_vecCenter.z,
        };
